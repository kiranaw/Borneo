...
to calculate-climb-cost [move-category]
  ...
  ;remember: terrestrial movement costs 2x climbing bouts (descent and climb)
    ...
    ;then, take the height of the target tree, plug into the energy formula
  ...
  [
    let climb-cost climb [height] of target-tree
...
to calculate-terrestrial-cost
  ...
  set move-cost walk distance target-tree
...
to find-route
    ask trees-here
    [
      set temp-path nw:turtles-on-path-to [target-tree] of myself
    ]
...
to from-csv
  ...
  while [ not file-at-end? ] [
    ...
    create-trees 1 [
      ...
      [set color red set size 2][set color green + 20]
   ...
   [ ;link with close neighbors
    ...
    [
      ...
      [
        ...
        [set link-type "sway" set color brown set thickness 0.1]
      ]
    ]
    ]
