  ...
  while [tree-counter < tree-density][
    while [col-counter < max-pxcor][
      ...
      while [row-counter < max-pycor][
        ...
        set row-counter row-counter + floor(max-pycor / sqrt(tree-density))
      ]
    set col-counter col-counter + floor(max-pxcor / sqrt(tree-density))
    ]
...
to generate-random
  nw:generate-random trees links number-of-trees connectivity [set color green + 20]
  repeat 200
  [
    let factor sqrt count trees
    if factor = 0 [ set factor 1 ]
    layout-spring trees links 0.2 100000 1
  ]
end
...
to small-world
  ...
  [
    let factor sqrt count trees
    if factor = 0 [ set factor 1 ]
    layout-spring trees links 0.2 factor / 2 1
  ]
end
